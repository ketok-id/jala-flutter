...
void main() {
  ...
  KetokDio.attach(dio);
  runApp(KetokOverlay(child: KetokExampleApp(dio: dio)));
}

/// Root of the example app.
class KetokExampleApp extends StatelessWidget {
  /// Creates the example app bound to [dio].
  const KetokExampleApp({required this.dio, super.key});

  /// Shared Dio instance with Ketok attached.
  final Dio dio;
...
class _DemoHomeState extends State<_DemoHome> {
  String _last = 'Tap a button to fire a request. Open Ketok via the K bubble.';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Ketok QA Rig'),
        actions: <Widget>[
          const IconButton(
            tooltip: 'Open Ketok inspector',
            icon: Icon(Icons.bug_report_outlined),
            onPressed: Ketok.open,
          ),
        ],
      ),
      body: ListView(
        ...
        children: <Widget>[
          ...
          _btn('POST json', () => _run('POST json', () async {
            await dio.post<dynamic>(
              ...
              data: <String, dynamic>{'hello': 'ketok', 'n': 42},
            );
          })),
          ...
          _btn('Bad host (error)', () => _run('bad host', () async {
            await dio.get<dynamic>('https://this-host-does-not-exist.ketok.dev/');
          })),
