...
/// A value whose [toString] blows up — used to simulate a bug in Ketok's
/// own capture logic (e.g. a header value that can't be stringified)
/// without ever touching `ketok_core`.
...
void main() {
  ...
  group('KetokDioInterceptor requests', () {
    test('captures a successful JSON GET request and response', () async {
      ...
      expect(entry.status, KetokCallStatus.success);
    ...
    test('captures a successful JSON POST with a request body', () async {
      ...
      final NetworkCallEntry entry = KetokBinding.instance.store.entries
          .single;
    ...
    test('redacts the Authorization header value in the store', () async {
      ...
      final String redacted = entry.requestHeaders.entries
          ...
          .value;
      expect(redacted, KetokRedactor.mask);
    ...
    test('captures a 500 error response as status error with body', () async {
      ...
      final NetworkCallEntry entry = KetokBinding.instance.store.entries
          .single;
      expect(entry.status, KetokCallStatus.error);
    ...
    test('captures cancellation as KetokCallStatus.cancelled', () async {
      KetokBinding.instance.initialize(config: KetokConfig(enabled: true));
      final harness = buildDio(
      ...
      final NetworkCallEntry entry = KetokBinding.instance.store.entries
          .single;
      expect(entry.status, KetokCallStatus.cancelled);
    });
    ...
    test('truncates a response body larger than maxBodyBytes', () async {
      KetokBinding.instance.initialize(
        config: KetokConfig(enabled: true, maxBodyBytes: 64),
      );
    ...
    test(
      ...
      'still succeeds',
      () async {
        KetokBinding.instance.initialize(config: KetokConfig(enabled: false));
        final harness = buildDio(
          (options) async => jsonResponseBody(<String, dynamic>{'ok': true}),
        );
        ...
        expect(response.statusCode, 200);
        await pump();
        expect(KetokBinding.instance.store.entries, isEmpty);
      },
    );
    ...
    test(
      ...
      'capture work',
      () async {
        // Deliberately never call KetokBinding.instance.initialize().
        final harness = buildDio(
          (options) async => jsonResponseBody(<String, dynamic>{'ok': true}),
        );
        ...
        expect(response.statusCode, 200);
        expect(KetokBinding.instance.isInitialized, isFalse);
      },
    );
