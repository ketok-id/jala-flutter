...
import 'ketok_themed_page.dart';

/// A draggable floating bubble showing a 'K' glyph and a badge with the
...
/// top-level widget.
class KetokOverlayButton extends StatefulWidget {
  ...
  @override
  State<KetokOverlayButton> createState() => _KetokOverlayButtonState();
}

class _KetokOverlayButtonState extends State<KetokOverlayButton>
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(
      ...
      child: GestureDetector(
        ...
        child: KetokThemedPage(
          child: _KetokBubble(diameter: widget.diameter),
        ),
      ),
    );
  }
...
class _KetokBubble extends StatelessWidget {
  const _KetokBubble({required this.diameter});

  final double diameter;

  @override
  Widget build(BuildContext context) {
    final Stream<List<NetworkCallEntry>> watch = KetokBinding.instance.isInitialized
        ? KetokBinding.instance.store.watch
    ...
    return StreamBuilder<List<NetworkCallEntry>>(
      ...
      builder: (BuildContext context, AsyncSnapshot<List<NetworkCallEntry>> snapshot) {
        ...
        final int pending = entries
            .where((NetworkCallEntry e) => e.status == KetokCallStatus.pending)
            .length;
        final int errors = entries
            .where(
              (NetworkCallEntry e) =>
                  e.status == KetokCallStatus.error ||
            ...
            children: <Widget>[
              ...
              if (errors > 0)
                Positioned(
                  ...
                  child: _CountBadge(
                    count: errors,
                    color: KetokTheme.serverErrorColor,
                  ),
                )
              else if (pending > 0)
