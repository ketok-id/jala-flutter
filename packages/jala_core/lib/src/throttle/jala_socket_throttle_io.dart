...
class JalaThrottledSocket extends StreamView<Uint8List> implements Socket {
  ...
  JalaThrottledSocket(Socket inner, JalaThrottleRegistry registry)
    : _inner = inner,
      _registry = registry,
      super(_pacedReads(inner, registry));
...
class JalaSocketThrottle {
  ...
  connectionFactory(JalaThrottleRegistry registry) {
    return (Uri url, String? proxyHost, int? proxyPort) async {
      ...
      if (!inScope) return Socket.startConnect(host, port);
      ...
      final ConnectionTask<Socket> task = await Socket.startConnect(host, port);
