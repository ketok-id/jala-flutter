...
///
/// Client integrations implement this — e.g. `ketok_dio` registers a
...
/// one is a no-op.
class KetokReplayRegistry {
  /// Creates an empty registry. Usually accessed via
  /// `KetokBinding.instance.replayRegistry`.
  KetokReplayRegistry();

  KetokReplayer? _replayer;
  ...
  /// one.
  void register(KetokReplayer replayer) {
    _replayer = replayer;
  }

  /// Unregisters [replayer] if it is the active one; otherwise does
  /// nothing.
  void unregister(KetokReplayer replayer) {
  ...
  Future<bool> replay(NetworkCallEntry entry) async {
    final KetokReplayer? replayer = _replayer;
