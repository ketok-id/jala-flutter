...
/// A parsed DevTools-style filter query over [NetworkCallEntry]s.
///
...
class KetokFilter {
  KetokFilter._(this.query, this._terms);

  /// Parses [query] into a filter. Never throws; malformed terms degrade
  /// to free-text terms.
  factory KetokFilter.parse(String query) {
    ...
    return KetokFilter._(query, terms);
  }
  ...
  static _Predicate _status(String value) {
    ...
    if (v == 'pending') {
      return (e) => e.status == KetokCallStatus.pending;
    }
    if (v == 'error') {
      return (e) =>
          (e.statusCode != null && e.statusCode! >= 400) ||
          e.status == KetokCallStatus.error ||
          e.status == KetokCallStatus.cancelled;
    }
