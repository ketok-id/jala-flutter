...
class KetokStore {
  KetokStore({required KetokEventBus bus, this.maxEntries = 300}) {
  ...
  StreamSubscription<KetokEvent>? _subscription;
  ...
  void _onEvent(KetokEvent event) {
    switch (event) {
  ...
  void _onRequest(NetworkRequestEvent e) {
    final NetworkCallEntry entry = NetworkCallEntry(
      ...
      status: KetokCallStatus.pending,
      client: e.client,
      requestSize: e.size,
      replayOf: e.replayOf,
    );
  ...
  void _onResponse(NetworkResponseEvent e) {
    ...
    final NetworkCallEntry updated = _entries[index].copyWith(
      ...
      duration: e.duration,
      responseSize: e.size,
      status: KetokCallStatus.success,
    );
  ...
  void _onCancel(NetworkCancelEvent e) {
    ...
    _entries[index] = _entries[index].copyWith(
      status: KetokCallStatus.cancelled,
    );
  }
  ...
  void _enforceCapacity() {
    while (_entries.length > maxEntries) {
      final int completedIndex = _entries.lastIndexWhere(
        (entry) => entry.status != KetokCallStatus.pending,
      );
      if (completedIndex != -1) {
