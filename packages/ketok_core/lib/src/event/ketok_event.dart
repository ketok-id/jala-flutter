...
/// Base type for everything flowing through [KetokEventBus].
///
/// Every event carries a `callId` used by `KetokStore` to correlate
/// request/response/error/cancel events for the same network call into a
/// single `NetworkCallEntry`.
sealed class KetokEvent {
  const KetokEvent({required this.callId, required this.timestamp});
...
class NetworkRequestEvent extends KetokEvent {
  ...
  /// event is the natural place for a replaying client (e.g. `ketok_dio`)
...
class NetworkResponseEvent extends KetokEvent {
...
class NetworkErrorEvent extends KetokEvent {
