...
void main() {
  late KetokEventBus bus;
  late KetokStore store;

  setUp(() {
    bus = enabledBus();
    store = KetokStore(bus: bus, maxEntries: 5);
  });
  ...
  group('correlation', () {
    test('request event creates a pending entry', () async {
      ...
      expect(entry.status, KetokCallStatus.pending);
    ...
    test('cancel event marks entry cancelled', () async {
      ...
      expect(store.byId('a')!.status, KetokCallStatus.cancelled);
    });
  ...
  group('store API', () {
    ...
    test('watch emits on every change', () async {
      ...
      expect(
        snapshots.any(
          (s) => s.any((e) => e.status == KetokCallStatus.success),
        ),
        isTrue,
      );
    });
  ...
  group('event bus', () {
    test('emit is a no-op while disabled', () async {
      var enabled = false;
      final gatedBus = KetokEventBus(isEnabled: () => enabled);
      final gatedStore = KetokStore(bus: gatedBus);
      addTearDown(() async {
  ...
  group('id generator', () {
    test('generates unique, non-empty ids', () {
      final ids = {for (var i = 0; i < 1000; i++) KetokIdGenerator.next()};
