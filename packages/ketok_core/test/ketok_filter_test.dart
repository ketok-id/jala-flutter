...
void main() {
  bool match(String query, NetworkCallEntry entry) =>
      KetokFilter.parse(query).matches(entry);
  ...
  group('malformed terms degrade to free text', () {
    ...
    test('parse never throws', () {
      for (final q in [
        ...
        expect(() => KetokFilter.parse(q).matches(makeEntry()), returnsNormally,
  ...
  group('empty query', () {
    test('matches everything', () {
      final filter = KetokFilter.parse('');
      expect(filter.isEmpty, isTrue);
      expect(filter.matches(makeEntry()), isTrue);
      expect(KetokFilter.parse('   ').matches(makeEntry()), isTrue);
    });
  });
}
